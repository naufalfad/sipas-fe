...
export default function SpatialCheckPanel({ submissionData }: SpatialCheckPanelProps) {
    ...
    useEffect(() => {
        ...
        const runLocalSpatialAudit = async () => {
        ...
        runLocalSpatialAudit();

        return () => {
            isMounted = false;
            window.dispatchEvent(new Event('map-clear-clash'));
        };
    }, [submissionData?.id, submissionData?.location?.polygon, validateRiverBuffer]);
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5 text-left">
                {isProcessing ? (
                    <div className="p-4 border border-slate-200 bg-slate-50 text-slate-500 flex items-center justify-center gap-2.5 rounded-none shadow-none select-none">
                        ...
                        <span className="text-[11px] font-black uppercase tracking-widest animate-pulse">Menghitung Geometri Sungai...</span>
                    </div>
                    ...
                    <div className="p-4 border text-[11px] font-semibold leading-relaxed text-justify rounded-none flex items-start gap-2.5 shadow-none select-none animate-in fade-in duration-300 text-rose-700 bg-rose-50 border-rose-200">
                        ...
                        <p className="leading-snug">Pelanggaran Spasial Terdeteksi! Ditemukan benturan kritis pada sempadan sungai seluas {auditResult.clashAreaSqm.toLocaleString('id-ID')} m². Berkas tidak dapat disetujui.</p>
                    </div>
                ) : (
                    <div className="p-4 border text-[11px] font-semibold leading-relaxed text-justify rounded-none flex items-start gap-2.5 shadow-none select-none animate-in fade-in duration-300 text-teal-700 bg-teal-50 border-teal-200">
                        ...
                        <p className="leading-snug">Hasil Verifikasi Spasial: Lahan bersih dari benturan spasial. Rencana mematuhi batas wilayah.</p>
                    </div>
                )}
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex flex-col border border-slate-200 rounded-none bg-white divide-y divide-slate-100 shadow-sm">
                        <div className={cn(
                            ...
                            !isProcessing && auditResult?.isClashing ? "bg-rose-50/30 border-rose-200 border-2 animate-pulse" : "hover:bg-slate-50/40"
                        )}>
                            <div className="space-y-1.5 min-w-0 flex-1">
                                <h5 className={cn("text-[11px] font-black leading-none", !isProcessing && auditResult?.isClashing ? "text-rose-700" : "text-slate-800")}>Cek Sempadan Sungai 25m</h5>
                                <p className={cn("text-[10px] leading-snug font-medium text-justify", !isProcessing && auditResult?.isClashing ? "text-rose-700" : "text-slate-500")}>
                                    {isProcessing ? "Menghitung validasi jarak 25 meter dari sungai terdekat..." : 
                                    auditResult?.isClashing ? `Melanggar: Batas kavling memotong area sempadan sungai sejauh 25m dengan area seluas ${auditResult.clashAreaSqm.toLocaleString('id-ID')} m²!` : 
                                    "Lolos: Batas kavling perumahan bersih dari zona aman sempadan sungai 25 meter."}
                                </p>
                            ...
                            <span className={cn(
                                ...
                                isProcessing ? "text-slate-500 bg-slate-50 border-slate-200" : auditResult?.isClashing ? "text-rose-700 bg-rose-50 border-rose-200 animate-pulse font-black" : "text-teal-700 bg-teal-50 border-teal-200"
                            )}>
                ...
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-none flex items-start gap-2.5 text-left select-none">
                    ...
                    <div className="space-y-1">
                        <h5 className="text-[9px] font-black uppercase tracking-widest leading-none text-slate-500">Keterangan Metode Spasial</h5>
                        <p className="text-[9px] font-semibold leading-normal text-slate-400 text-justify">
                            Pemeriksaan spasial dijalankan otomatis via point-in-polygon overlay menggunakan data acuan RTRW dan RDTR resmi Kabupaten Bogor tahun 2025 dengan Turf.js murni.
                        </p>
                    </div>
                </div>
            </div>
